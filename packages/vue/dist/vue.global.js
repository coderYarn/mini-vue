var MiniVue = (() => {
  ...
  function normalizeVNode(result) {
    ...
    return h(Text2, null, result.toString());
  }
  ...
  function resolveElementASTVNode(node, parent) {
    ...
    if (ifNode) {
      ...
      const { exp } = ifNode;
      return `${exp.content}?${consequent}:${alternate || (alternate = createTextVNode(Text))}`;
    }
    if (forNode) {
      const { exp } = forNode;
  ...
  function pluck(directives, name, remove = true) {
    const index = directives.findIndex((dir2) => dir2.name === name);
  ...
  function resolveComponent(name) {
    return components && (components[name] || camelize(components[name]) || capitalize(camelize(components[name])));
  }
